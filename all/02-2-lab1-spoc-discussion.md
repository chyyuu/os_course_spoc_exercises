...
>  首先实现一个能显示字符串的bootloader，需注意字符编码方式（UTF-8）。然后再实现能加载ELF文件的bootloader，这时首先应该检测该文件是否是ELF文件，且需要注意ELF文件的大小不能超过限制。


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

>  首先注册一个新的系统调用（系统调用表文件中添加此项，再到相应文件中添加对应的宏），其功能是能够在屏幕上输出所有系统调用的调用次数。然后实现这一功能对应的函数。在实现该函数的文件中建立一个全局数组count来实现系统调用次数的存储且能保证在每次开机时其被初始化为0，系统调用在其中的排序和系统调用表中系统调用的顺序相同，这样能方便计数。之后在系统调用的汇编语句之前增加一个自增语句，使得每一次系统调用的时候count中与该系统调用相对应的位置的项加1，即实现系统调用计数。
