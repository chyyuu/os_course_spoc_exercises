...
>  操作系统初始化GDT，完成段机制。
   探测物理内存，管理空闲物理内存。
   页表建立初始化和设置CR0控制寄存器的最高位置1来使能页机制。
...
> 有差异，我的学号为2012011393，余数为11。加入这一行代码之后人工产生了中断，qemu中保存了中断现场，11是中断返回的编号。
...
```
修改lab2，让其显示` type="some string"` 而不是单纯的数字1,2  (easy) 
- [x]  

> 在memlayout.h中的map结构体中的type对应的值为：
 #define E820_ARM 1
 #define E820_ARR 2
