...
```
 ...
 ```
- [x]  
	初始化8259A中断控制器，通过端口设置主从关系和中断屏蔽方式
	初始化中断向量表，设置每个中断（包括时钟）的中断处理函数地址
	初始化8253时钟，设置时钟模式和频率，并在中断控制器中使能时钟中断
...
- [x]  
通过数组记录系统调用的次数，每次在中断处理程序中增加计数器，通过全局变量即可获取调用次数。
> 
