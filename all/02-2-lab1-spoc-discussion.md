...
>  首先初始化IDT，然后初始化8259中断控制器，然后初始化8253时钟外设
...
void test(int i, ...)
{
	va_list get;
	va_start(get, i);
	int j = va_arg(get, int);
	int k = va_arg(get, int);
	va_end(get);
...
> 可以先实现一个可显示字符串的bootloader，再实现一个可加载ELF格式文件的bootloader


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> 创建一个数组用于分别记录每个系统调用的次数，每个调用号唯一确定一个元素。找到进行调用的汇编语句（比如sys_call（%eax），%eax是调用号），在前面添加一个步骤，就是用系统调用号%eax来找到这个系统调用对应的数组的元素，然后让这个元素自增1.（比如添加这句话increase mycount（%eax），意即找到%eax对应的元素然后加一）
