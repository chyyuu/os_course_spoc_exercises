...
>  中断描述符表（Interrupt Descriptor Table） 中断描述符表把每个中断或异常编号和一个指向中断服务例程的描述符联系起来。CPU把中断（异常）号乘以8做为IDT的索引。IDT可以位于内存的任意位置，CPU通过IDT寄存器（IDTR）的内容来寻址IDT的起始地址。
...
> 先实现一个可现实字符的bootloader，要处理时钟，串口，CGA等外设。 再实现一个可加载ELF格式文件的bootloader，要处理硬盘中断。

如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> (1) 使用strace跟踪系统调用信息. 
  (2) 系统调用要访问IDT，记录系统调用信息，将其写入内核。

如何修改lab1, 实现一个可显示字符串"THU LAB1"且依然能够正确加载ucore OS的bootloader？如果不能完成实现，请说明理由。
- [x]  

> bootloader的作用是加载ucore OS,,这段代码只有不超过510字节的大小,无法容纳多余的代码,并且不能处理外设,因此无法实现.
